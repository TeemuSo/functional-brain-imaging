...
$gender = "male"; # Gender is either 'male' or 'female'

# One stimuli + isi takes 2s approximately
# Break between blocks is 2.5s
$isi_min 			= 1000;
$isi_max 			= 2000;
$n_pictures_block = 1; 	# How many similar images each block? (same race, same pain / no-pain)
$n_blocks 			= 85; # How many of these blocks in one session? (62)
$n_sessions			= 3; 	# How many sessions?

# How many stimuli in the arrays?
# UPDATE THIS TO REAL VALUE
...
# MEG
pulse_width = 10;    # Duration of the output trigger pulse in ms         
write_codes = true; # False while testing
...
array {
	LOOP $i $n_pictures_block;
		picture { bitmap { preload = false; transparent_color = 255,255,255; }; x = 0; y = 0; };
	ENDLOOP;
} pic;
...
begin
	...
	begin
		...
		string code = stimulus_name + string(i) + "_pain_" + string(is_pain_picture);
		...
		if (n_pictures_block == 1) then
			...
			event.set_port_code( similarity_value );
...
sub array<bitmap, 1> create_stimulus_block ( string category, bool is_pain_picture )
#######
...
begin
	...
	start_txt.set_caption("Is the person in pain or not?", true);
	...
	begin	
		...
		bool is_pain_picture;
		double rnd = random();
		if (rnd < pain_no_pain_ratio) then
			is_pain_picture = true;
		else
			is_pain_picture = false;
		end;
		...
		double prob = 4 / n_pictures_presented;
		...
		# Add question mark before our new block
		if ( rnd < prob ) then
		...
		out.print_line("Chosen ethnicity is " + chosen_category + ". Pain: " + string(is_pain_picture));
			
		# Choose the properties that we want for this block
		array<bitmap> bmp_array[n_pictures_block] = create_stimulus_block( chosen_category, is_pain_picture );
		initialize_stimulus_block( bmp_array, chosen_category, delta_time, is_pain_picture );
